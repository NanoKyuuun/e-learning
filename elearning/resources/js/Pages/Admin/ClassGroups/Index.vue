...
const deleteClass = (id) => {
    if (confirm('Apakah Anda yakin ingin menghapus kelas ini?')) {
        router.delete(route('admin.class-groups.destroy', id));
    }
};

watch(search, debounce((value) => {
    router.get(route('admin.class-groups.index'), { search: value }, {
        preserveState: true,
        replace: true,
    });
}, 300));
    ...
    <AdminLayout>
        <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-base-content/60">Kelola data kelas dan daftar siswa di dalamnya secara global.</p>
            </div>
            <Link :href="route('admin.class-groups.create')" class="btn btn-primary shadow-lg shadow-primary/20">
                        ...
                        <tr v-for="item in classGroups.data" :key="item.id" class="hover">
                            ...
                            <td class="text-center">
                                <div class="flex justify-center gap-2">
                                    <Link :href="route('admin.class-groups.members.index', item.id)" class="btn btn-sm btn-primary gap-2 shadow-sm" title="Kelola Anggota Kelas">
                                        <Users class="w-4 h-4" /> Kelola Siswa
                                    </Link>
                                    <div class="join shadow-sm border border-base-200">
                                        <Link :href="route('admin.class-groups.edit', item.id)" class="btn btn-sm btn-ghost join-item border-r border-base-200 hover:text-info">
