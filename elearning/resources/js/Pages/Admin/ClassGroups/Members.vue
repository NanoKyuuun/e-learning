...
watch(search, debounce((value) => {
    router.get(route('admin.class-groups.members.index', props.classGroup.id), { search: value }, {
        preserveState: true,
        replace: true,
    });
}, 300));
...
const enroll = () => {
    ...
    form.post(route('admin.class-groups.members.store', props.classGroup.id), {
        onSuccess: () => {
...
const removeStudent = (enrollmentId) => {
    if (confirm('Keluarkan siswa ini dari kelas?')) {
        router.delete(route('admin.class-enrollments.destroy', enrollmentId));
    }
};
</script>
...
<template>
    ...
    <AdminLayout>
        <div class="mb-8">
            <Link :href="route('admin.class-groups.index')" class="btn btn-ghost btn-sm gap-2 mb-4">
