...
const submit = () => {
    form.post(route('admin.teaching-assignments.schedules.store', props.teachingAssignment.id), {
        onSuccess: () => {
...
const deleteSchedule = (id) => {
    if (confirm('Hapus jadwal ini?')) {
        router.delete(route('admin.class-schedules.destroy', id));
    }
};
...
<template>
    ...
    <AdminLayout>
        <div class="mb-8">
            <Link :href="route('admin.teaching-assignments.index')" class="btn btn-ghost btn-sm gap-2 mb-4 font-bold uppercase text-[10px] tracking-widest">
