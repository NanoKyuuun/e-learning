...
import { UserPlus, Pencil, Trash2, Search, Plus, BookOpen, School, UserCircle, Calendar } from 'lucide-vue-next';
...
const deleteAssignment = (id) => {
    if (confirm('Hapus plotting pengampu ini?')) {
        router.delete(route('admin.teaching-assignments.destroy', id));
    }
};

watch(search, debounce((value) => {
    router.get(route('admin.teaching-assignments.index'), { search: value }, {
        preserveState: true,
        replace: true,
    });
}, 300));
    ...
    <AdminLayout>
        <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-base-content/60">Hubungkan Guru, Mata Pelajaran, dan Kelas secara global.</p>
            </div>
            <Link :href="route('admin.teaching-assignments.create')" class="btn btn-primary shadow-lg shadow-primary/20">
                        ...
                        <tr v-for="(item, index) in assignments.data" :key="item.id" class="hover">
                            ...
                            <td class="text-center">
                                <div class="flex justify-center gap-1">
                                    <Link :href="route('admin.teaching-assignments.schedules.index', item.id)" class="btn btn-sm btn-square btn-ghost hover:bg-info/10 hover:text-info" title="Kelola Jadwal">
